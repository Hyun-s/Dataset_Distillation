...
#     out = []
#     for x in range(distributed):
#         tmp = torch.randn(1).to('cuda:'+str(x))
#         out.append(tmp)
#     return out


# def repram(model, st):
#     for p, p_t in zip(model.parameters(), st[0][-1]):
#         p.data = p_t
#     for p, p_t in zip(model.parameters(), st[0][-1]):
#         if torch.all(torch.all(p.data == p_t)):
#             continue
#         else:
...
#         syn_label.append(torch.ones(ipc)*c)

#     syn_label = torch.cat(syn_label)

#     out = out.detach().cpu().numpy()
#     target = syn_label.flatten().numpy()
#     i = np.concatenate([samples,out])
#     l = np.concatenate([samples_l,target])
#     tsne = TSNE(perplexity=50.0)

#     out_2d = tsne.fit_transform(i)

#     if it > 1:
#         plt.clf()
#     plt.scatter(out_2d[:k,0], out_2d[:k,1], c= l[:k], alpha=0.2)
...
def main(args):
    ...
    st = torch.load(path)
    model = repram(model,st).to(args.device)
    loader = torch.utils.data.DataLoader(dst_train, batch_size=128, shuffle=False, num_workers=2)
    ...
    label_ = torch.cat(label)
    # feat_.shape
    perm = torch.randperm(feat_.size(0))
    idx = perm[:k]
    samples = feat_[idx]
    samples_l = label_[idx]
    ...
    # Synthetic data and synthetic learningrate
    ...
    for it in range(0, args.Iteration+1):
        ...
        if it in eval_it_pool and (save_this_it or it % 1000 == 0):
            ...
            with torch.no_grad():
                ...
                if args.ipc < 50 or args.force_save:
                    ...
                    log_dict = {}
                    for idx, x in enumerate(visualization):
                        wandb.log({"Synthetic_CAM_"+str(idx): x},step=it)
                    ...
                    #     error_message = str(e)
                    #     with open("/workspace/hyuns/src/dc_benchmark/methods/tm/error_log.txt", "w") as file:
        ...
        student_net = get_network(args.model, channel, num_classes, im_size, dist=False, args=args).to(args.device)  # get a random model
...
if __name__ == '__main__':
    ...
    parser.add_argument('--canvas_size', type=int, default=1, help='size of synthetic canvas')
    ...
    parser.add_argument('--tsne_model',  type=str, default='/workspace/hyuns/src/dc_benchmark/methods/tm/buffer_images/cifar_10/CIFAR10/ConvNet/replay_buffer_18.pt')
