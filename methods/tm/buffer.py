...
def main(args):
    ...
    for it in tqdm(range(0, args.num_experts), desc='N_Experts', position=0):

        ''' Train synthetic data '''
        teacher_net = get_network(args.model, channel, num_classes, im_size, args=args).to(args.device) # get a random model
        teacher_net.train()
        lr = args.lr_teacher
        ...
        for e in tqdm(range(args.train_epochs), desc='Epochs', position=1, leave=False):
            ...
            # tqdm.set_description("Itr: {}\tEpoch: {}\tTrain Acc: {}\tTest Acc: {}".format(it, e, train_acc, test_acc))
            timestamps.append([p.detach().cpu() for p in teacher_net.parameters()])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', type=str, default='8', help='Select GPU')
