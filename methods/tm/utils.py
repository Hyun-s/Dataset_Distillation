...
def min_max(input_tensor):
    """
    input_tensor (BCHW)
    """
    M = input_tensor.max(2,keepdim=True)[0].max(3,keepdim=True)[0]
    m = input_tensor.min(2,keepdim=True)[0].min(3,keepdim=True)[0]
    return (input_tensor - m) / (M-m)
...
def get_cam(model, input_tensor, num_classes, target_layers, use_cuda):
    ...
    for label in range(num_classes):
        ...
        vi = []
        for rgb,inputs in zip(rgb_img, input_tensor):
            grayscale_cam = cam(input_tensor=inputs.unsqueeze(0), targets=targets)
            grayscale_cam = grayscale_cam[0, :]
            visualization = show_cam_on_image(rgb, grayscale_cam, use_rgb=True)
            vi.append(visualization[np.newaxis,:,:,:])
...
def get_network(model, channel, num_classes, im_size=(32, 32), dist=False, args=None):
